#' @title Computing Region stdev
#'
#' @description Computes the stdev of the MRI voxel values for each patient
#' stratified on individual clusters.
#'
#' @details \code{computeRegionSD()} takes in two arguments: a \code{tibble}
...
#' \code{computeRegionSD()} combines \code{voxel_df} and \code{data_df} into
#' a single \code{tibble}, and then pivots this combined data with respect to
#' the columns 'pid' and 'cluster'. We then find the stdev of each pid-cluster
...
#' equal to \code{NULL}, \code{computeRegionSD()} will assume that all data comes
#' from the same cluster.
#'
#' @return A tibble with the following components:
#' \describe{
#' \item{pid}{a column indicating patient identification}
#' \item{sd_c}{columns indicating the calculated stdev of the patient for
...
computeRegionSD <- function(voxel_df, data_df = NULL) {
  ...
  if (!is.null(data_df)) {
    ...
    result <- combine_df %>%
      ...
      summarize(sd = sd(value)) %>% # obtaining stdev according to cluster and pid
      pivot_wider(names_from = "cluster",
                  values_from = "sd",
                  names_prefix = "sd_c") # reformatting the tibble to be in wide-format
  }
    ...
    result <- voxel_df %>%
      group_by(pid) %>%
      summarize(sd = sd(value)) # if data_df is not provided, we don't take into consideration the cluster labels and calculate stdev directly on each pid
  }
