...
dataDrivenClusters <- function(voxel_df, n_pca = 20, n_umap = 2, n_clust = 2,
  ...
  umap_sim <- umap(pcs, n_components = n_umap, preserve.seed= TRUE)
  ...
  data_df <- cbind(cbind(voxel_df, umap_coords), km$cluster)

  #obtain a dataframe with umap coords and kmeans clusters to make plotting easier

  plot_df <- cbind(umap_coords, km$cluster)
  colnames(plot_df) <- c('umap coord 1', 'umap coord 2', 'clusters')
  plot <- ggplot(plot_df, aes(x = 'umap coord 1', y = 'umap coord 2', color = 'clusters'))
  + geom_point()
